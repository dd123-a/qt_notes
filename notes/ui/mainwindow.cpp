...
MainWindow::MainWindow(Storage* storage) : QMainWindow(nullptr), ui(new Ui::MainWindow) {
    ...
    _historyButton=new QToolButton();
    _historyButton->setPopupMode(QToolButton::MenuButtonPopup);
    _historyButton->setMenu(new QMenu());
    _historyButton->setText("历史");
    connect(_historyButton, &QToolButton::clicked, this, &MainWindow::showHistory);
    ui->mainToolBar->insertWidget(ui->mainToolBar->actions()[4], _historyButton);
...
void MainWindow::showHistory() {
    ...
    // 假设 historyFiles 是一个存储历史文件路径的 QVector

    // 添加历史文件到菜单项
    for (const QString& filePath : historyFiles) {
        QAction* historyAction = new QAction(filePath);
        historyMenu->addAction(historyAction);

        // 连接历史文件项的点击事件到相应的槽函数
        connect(historyAction, &QAction::triggered, this, [this, filePath]() {
            onHistoryFileClicked(filePath);
        });
    }

    // 显示菜单
    QPoint historyButtonPos = _historyButton->geometry().bottomLeft();
    historyMenu->exec(mapToGlobal(historyButtonPos));
}

void MainWindow::onHistoryFileClicked(const QString& filePath) {
    qDebug() << "Jump to file: " << filePath;
    selectFile(filePath);
}
...
void MainWindow::onTabChanged() {
    ...
    if(exists){
        ...
        // 检查是否已经包含相同的文件名
        if (!historyFiles.contains(fileName)) {
            historyFiles.append(fileName);
        } else {
            qInfo() << "File already exists in history: " << fileName;
        }
    }
