...
void Setup::nativeAutostartAdd() {
    ...
    HKEY runKey = nullptr;
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Run", 0, KEY_WRITE, &runKey) == ERROR_SUCCESS) {
        QByteArray buffer = value.toUtf8();
        RegSetValueExW(runKey, reinterpret_cast<LPCWSTR>(valueName.utf16()), 0, REG_SZ,
                       reinterpret_cast<const BYTE*>(buffer.constData()), static_cast<DWORD>(buffer.size()));
        RegCloseKey(runKey);
    }
}
...
void Setup::nativeAutostartRemove() {
    ...
    HKEY runKey = nullptr;
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Run", 0, KEY_WRITE, &runKey) == ERROR_SUCCESS) {
        RegDeleteValueW(runKey, reinterpret_cast<LPCWSTR>(valueName.utf16()));
        RegCloseKey(runKey);
    }
}
...
bool Setup::nativeAutostartCheck() {
    ...
    bool isFound = false;
    HKEY runKey = nullptr;
    if (RegOpenKeyExW(HKEY_CURRENT_USER, L"Software\\Microsoft\\Windows\\CurrentVersion\\Run", 0, KEY_READ, &runKey) == ERROR_SUCCESS) {
        wchar_t valueBuffer[4096];
        DWORD valueBufferSize = sizeof(valueBuffer);
        if (RegGetValueW(runKey, nullptr, reinterpret_cast<LPCWSTR>(valueName.utf16()), RRF_RT_REG_SZ, nullptr, valueBuffer, &valueBufferSize) == ERROR_SUCCESS) {
            QString value = QString::fromUtf16(reinterpret_cast<ushort*>(valueBuffer), static_cast<int>(valueBufferSize) / 2 - 1);
