...
class FileItem::SyntaxHighlighter : public QSyntaxHighlighter {
public:
    SyntaxHighlighter(QTextDocument* parent = nullptr) : QSyntaxHighlighter(parent) {
        ...
        bracesFormat.setForeground(Qt::gray); // 设置 {} 的颜色为黄色
        bracesFormatA.setForeground(Qt::gray);

        // 定义关键字列表
        QStringList keywords;
        keywords << "\\bif\\b" << "\\belse\\b" << "\\bfor\\b" << "\\bwhile\\b" << "\\breturn\\b";

        // 生成匹配关键字的正则表达式
        QString keywordPattern = "\\b(" + keywords.join("|") + ")\\b";
        keywordExpression.setPattern(keywordPattern);
    }

protected:
    void highlightBlock(const QString& text) override {
        // 在文本块中进行语法高亮
        highlightMatches(text, commentExpression, commentFormat);
        highlightMatches(text, keywordExpression, keywordFormat);
        highlightMatches(text, stringExpression, stringFormat);
        highlightMatches(text, numberExpression, numberFormat);
        highlightMatches(text, bracesExpression,bracesFormat); // 添加 {} 的高亮方法
        highlightMatches(text, bracesExpressionA,bracesFormatA);
    }
...
private:
    ...
    QTextCharFormat bracesFormatA;
    QTextCharFormat bracesFormat; // 添加用于匹配 {} 的格式

    QRegularExpression commentExpression{R"((//.*?$)|(/\*.*?\*/))"};
    QRegularExpression keywordExpression;
    QRegularExpression stringExpression{R"("[^"]*"|'[^']*')"};
    QRegularExpression numberExpression{R"(\b\d+\.?\d*\b)"};
    QRegularExpression bracesExpressionA{R"(\(|\))"};
    QRegularExpression bracesExpression{R"({|})"}; // 添加匹配 {} 的正则表达式

    void highlightMatches(const QString& text, const QRegularExpression& expression, const QTextCharFormat& format) {
    ...
    void highlightBraces(const QString& text, const QTextCharFormat& format) {
        QRegularExpressionMatchIterator matchIterator = bracesExpression.globalMatch(text);
        while (matchIterator.hasNext()) {
            QRegularExpressionMatch match = matchIterator.next();
            setFormat(match.capturedStart(), match.capturedLength(), format);
        }
...
FileItem::FileItem(FolderItem* folder, QString fileName)
    ...
    _folder = folder;
    _fileName = fileName;
    static QRegularExpression re(".*html.*");
    if(re.match(fileName).hasMatch()){
    _highlighter = new SyntaxHighlighter(document());
    }
...
QString fileTypeToString(FileType fileType) {
    switch (fileType) {
    case FileType::Plain:
        return "Plain";
    case FileType::Html:
...
bool FileItem::load() {
    ...
    if (file.exists()) {
        if (file.open(QIODevice::ReadOnly)) {
            ...
            qDebug() << "File type:" << fileTypeToString(type());
            switch (type()) {
#if QT_VERSION >= QT_VERSION_CHECK(5, 14, 0)
                case FileType::Markdown:
